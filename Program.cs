...
using conversation_audit_service.Adapters.Inbound.Http;
...
using conversation_audit_service.Application.UseCases;
using conversation_audit_service.Configuration;
...
builder.Services.AddOpenTelemetry()
    .ConfigureResource(resource => resource.AddService("conversation-audit-service"))
...
builder.Services.AddScoped<IJourneyEventRepository, PostgresJourneyEventRepository>();
builder.Services.AddScoped<IRecordJourneyEventUseCase, RecordJourneyEventUseCase>();
    ...
    CancellationToken cancellationToken) =>
{
    ...
    if (!InternalAuthOptions.HasValidSecret(
            authOptions.Value.InboundSecrets.GetValueOrDefault("conversation-orchestrator")))
    {
        failures.Add("internal_auth_inbound_secret_missing:conversation-orchestrator");
    }
    try
    {
